...
# Example location fix
Patch6: hdf5-example.patch
# Java test location fix
Patch7: hdf5-javatest.patch
...
# Fix test output
#junit_ver=$(sed -n '/<modelVersion>/{s/^.*>\([0-9]\.[0-9]*\)<.*/\1/;p;q}' /usr/share/maven-poms/JPP-junit.pom)
junit_ver='4.11'
...
%global configure_opts \\\
  ...
  --enable-hl \\\
  --enable-shared \\\
  --with-szlib=/usr/include,/usr/lib64 \\\
...
do
  ...
  %configure \
    ...
    LDFLAGS="-L/usr/lib64/$mpi/lib/" LD_LIBRARY_PATH="/usr/lib64/$mpi/lib/"
  sed -i -e 's! -shared ! -Wl,--as-needed\0!g' libtool
...
# Commenting out the H5pubconf multiarch as netcdf failed to look in to different files
...
%{_bindir}/h5unjam
%{_bindir}/h5watch
%{_libdir}/libhdf5.so.200*
%{_libdir}/libhdf5_cpp.so.200*
%{_libdir}/libhdf5_fortran.so.200*
%{_libdir}/libhdf5hl_fortran.so.200*
%{_libdir}/libhdf5_hl.so.200*
%{_libdir}/libhdf5_hl_cpp.so.200*
...
%{_libdir}/%{mpich}/lib/*.so.20*

%files mpich-devel
%{_includedir}/%{mpich}-%{_arch}
%{_fmoddir}/%{mpich}-%{_arch}/*.mod
...
%changelog
* Thu Jul 30 2020 Ketan Patel <k2patel@live.com> - 1.12.0-1
- mpich to 3.2 and openmpi3
- Patches for the newer version
- falling back to szip instead of libaec
